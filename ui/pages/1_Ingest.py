...
with st.form("ingest_form"):
    ...
    submitted = col1.form_submit_button(
        "🚀 Start Ingestion", use_container_width=True
    )
...
if submitted:
    if not repo_url and not local_path:
        ...
        with st.status("Ingesting repository...", expanded=True) as status:
            ...
            st.write("🔄 Cloning repository (shallow clone)...")
        ...
        if result["success"]:
            ...
            st.success(f"Repository **{data.get('repo_name')}** indexed successfully!")
            ...
            col4.metric("🗂️  Index Type", data.get("index_type", "flat").upper())
...
# ── Popular repos quick-pick ───────────────────────────────────────────────────
st.subheader("🌟 Quick Examples")
st.caption("Click to copy URL into the form above.")
...
for col, (name, url, branch) in zip(cols, examples):
    with col:
        st.markdown(f"**{name}**")
