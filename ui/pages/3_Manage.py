    ...
    for repo in repos:
        with st.expander(f"📚 {repo}", expanded=True):
            ...
            if st.button(f"🗑️ Delete '{repo}'", key=f"del_{repo}", type="secondary"):
                ...
                if result["success"]:
                    st.success(f"Deleted '{repo}' successfully.")
                    st.rerun()
                else:
...
if cache:
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Cached Answers", cache.get("size", 0))
    col2.metric("Hit Rate",       f"{cache.get('hit_rate_pct', 0)}%")
    col3.metric("Total Hits",     cache.get("hits", 0))
    col4.metric("Total Misses",   cache.get("misses", 0))
...
if health.get("status") == "ok":
    ...
    col1.metric("LLM Provider",    health.get("llm_provider", "—").upper())
    col2.metric("Embedding Model", health.get("embedding_model", "—").split("/")[-1])
